...
# 과거 별도 GV/무대인사 웹훅을 쓰던 저장소도 그대로 호환한다.
...
def detect_format(row):
    ...
    if (
        ...
        or str(row.get("scnsNo") or "").strip()
        == YONGSAN_4DX_SCNS_NO
    ):
        return "4DX"
...
def alert_ticket_line(event):
    ...
    # 회차 줄 전체를 클릭 가능한 CGV 예매 링크로 만든다.
    if link:
        return f"[{text}]({link})"
...
def send_event_alert_group(events, alert_kind):
    """
    같은 날짜 + 같은 종류 + 같은 상태 변화의 회차를
    Discord 메시지 하나로 묶어서 전송한다.
    """
    ...
    header.extend([
        ...
        f"🎬 {SITE_NAME} · {event_type}",
        f"📅 {pretty_date(date)}",
    ])

    # Discord 2000자 제한 대비.
    # 같은 그룹이 너무 길 때만 제목을 유지한 채 자동 분할한다.
    current = list(header)
    sent_any = False

    for event in events:
        line = alert_ticket_line(event)
...
def _prepare_event_transition(
    ...
    반환:
      ("ALERT", alert_kind, target_status)
      ("STATE", None, target_status)
      ("IGNORE", None, None)
    """
...
def process_events(
    ...
    """
    한 날짜의 API 응답 전체를 한꺼번에 처리한다.

    같은 날짜 + 같은 종류 + 같은 상태 변화는
    Discord 메시지 하나로 묶는다.
    """
    ...
    for key, event in events.items():
        action, alert_kind, target_status = (
            _prepare_event_transition(
                key,
                event,
                seen,
                booking_state,
    ...
    # Discord 알림이 필요 없는 내부 상태 갱신
    for (
        key,
        event,
        target_status,
    ) in immediate_updates:
        previous = (
            booking_state.get(key)
            or {}
        )
...
def initialize_missing_state(
    ...
    need_booking_baseline = (
        not booking_state_ready
    )

    if (
        not need_seen_baseline
        and not need_booking_baseline
    ):
        return (
            seen,
            booking_state,
    ...
    print("=" * 72)
    print(
        f"INITIAL {DAYS}-DAY BASELINE"
    )
    print("=" * 72)

    if (
        need_seen_baseline
        and need_booking_baseline
    ):
        print(
            f"현재 {DAYS}일 전체 GV / 무대인사 / "
            "IMAX / 4DX와 예매 상태를 "
            "알림 없이 기준값으로 등록합니다."
        )
    ...
    events, error = collect_full_scan(
        session,
        progress=True,
    )
...
def has_priority_state_for_date(
    date,
    booking_state,
):
...
def interval_for_date(
    ...
    base = base_interval_for_offset(
        offset
    )

    if has_priority_state_for_date(
        date,
        booking_state,
    ):
        return min(
...
def stagger_schedule(
    dates,
    booking_state,
    start_at=None,
):
    """
    같은 주기의 날짜들이 한 순간에 몰리지 않도록
    고르게 분산한다.
    """
...
def make_fast_scan_dates():
    ...
    return [
        ...
        for offset in range(
            FAST_SCAN_START_OFFSET,
            FAST_SCAN_END_OFFSET + 1,
        )
    ]


def run_0030_fast_scan(
    seen,
    booking_state,
    date_event_cache,
):
    ...
    print(
        f"{icon} "
        f"{now_kst().strftime('%H:%M')} "
        "00/30 동시스캔 완료 | "
        f"+4~+21일 | "
        f"성공 {success}/{len(dates)} | "
        f"{elapsed:.2f}초 | "
        f"Discord 알림 {alerts} | "
        f"오류 {errors}"
    )
...
def run_initial_monitor_scan(
    session,
    seen,
    booking_state,
):
    """
    감시 시작 시 현재 감시 범위를 딱 한 번 훑어
    캐시를 만든다.
    """
...
def run_monitor(
    ...
    while (
        ...
        # 매시 00/30분에는 +4~+21일 18일을
        # 한 번에 동시 확인한다.
        wall_now = now_kst()

        if (
            wall_now.minute
            in FAST_SCAN_MINUTES
        ):
            slot_key = (
                wall_now.strftime(
                    "%Y%m%d%H%M"
            ...
            if (
                ...
                fast_count = len(
                    make_fast_scan_dates()
                )

                total_requests += fast_count
                window_requests += fast_count
        ...
        window_success += 1
        date_event_cache[due_date] = (
            events
        )

        # ★ 개별 회차 process_event 사용 금지
        # 날짜 전체를 한 번에 묶어서 처리
        cycle_alerts, cycle_soldout = (
            process_events(
                events,
                seen,
                booking_state,
...
def main():
    ...
    print(
        f"MIN REQUEST START GAP: "
        f"{MIN_REQUEST_GAP:.2f}s"
    )
    ...
    try:
        ...
        if r.status_code != 200:
            print(
                "⚠️ CGV BOOKING PAGE CHECK FAILED - "
                "API 감시는 계속합니다."
            )

    except Exception as e:
        print(
            "⚠️ CGV BOOKING PAGE CHECK WARNING:",
            repr(e),
        )

        print(
            "BOOKING PAGE CHECK FAILED - "
            "API 감시는 계속합니다."
        )
    ...
    if (
        ...
        print(
            "RUN TIME FINISHED AFTER BASELINE"
        )
        return
